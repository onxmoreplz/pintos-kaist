...
tid_t process_create_initd(const char *file_name) {
  ...
  // file_name을 이름으로 하고 PRI_DEFAULT를 우선순위로 갖는 새로운 thread 생성,
  // tid에 저장 thread는 fn_copy를 인자로 받는 initd라는 함수를 실행시킴
...
static bool duplicate_pte(uint64_t *pte, void *va, void *aux) {
  ...
  if (is_kernel_vaddr(va)) {
    return true;  // return false ends pml4_for_each, which is undesirable -
                  // just return true to pass this kernel va
  }
  ...
  if (!pml4_set_page(current->pml4, va, newpage, writable)) {
    ...
    printf(
        "Failed to map user virtual page to given physical frame\n");  // #ifdef
                                                                       // DEBUG
    return false;
  }
...
static void __do_fork(void *aux) {
  ...
  if (!pml4_for_each(parent->pml4, duplicate_pte,
                     parent))  // to copy entire user memory space including
                               // corresponding pagetable structures
    goto error;
#endif
  ...
  for (int i = 0; i < FDCOUNT_LIMIT; i++) {
    ...
    // If 'file' is already duplicated in child, don't duplicate again but share
    // it
    bool found = false;
    if (!found) {
...
int process_wait(tid_t child_tid UNUSED) {
  ...
  sema_up(&child->free_sema);  // wake-up child in process_exit - proceed with
                               // thread_exit
  return exit_status;
}
