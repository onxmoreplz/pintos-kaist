...
/* Pintos main program. */
/**
 * 즉, Pintos가 시작되면 이 함수가 실행
 * 메모리, 스레드, page table 등을 초기화 해주는 작업이 진행
*/
int main(void)
{
	uint64_t mem_end;
...
static void run_task(char **argv)
{
	...
	printf("Executing '%s':\n", task);
#ifdef USERPROG
	if (thread_tests) // Boolean main() 함수에서 argv를 파싱할 때, 조건에 따라 True로 바꿈
	{
		run_test(task); // 프로젝트 1에서 test 돌린 결과
	}
	else // 사용자 프로그램
	{
...
static void run_actions(char **argv)
{
	...
	static const struct action actions[] = {
		{"run", 2, run_task}, // "run"이라는 action이 들어오면 run_task라는 함수가 실행
#ifdef FILESYS
		{"ls", 1, fsutil_ls},
		{"cat", 2, fsutil_cat},
		{"rm", 2, fsutil_rm},
	...
	while (*argv != NULL) 
	{
		...
		/* Find action name. */
		for (a = actions;; a++) // action의 구조체를 탐색하면서 인자로 받은 argv와 action name 을 비교
			if (a->name == NULL)
