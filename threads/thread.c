...
static int64_t
    next_tick_to_awake;  // wakeup_tick 값 중 최소값 저장하기 위한 변수
...
bool thread_cmp_donate_priority(const struct list_elem *a,
...
/**
 * thread_test_preemption - ready_list의 가장 앞에있는 스레드의 우선순위와 현재
 * running상태 스레드와 우선순위 비교
 */
...
void update_next_tick_to_awake(int64_t ticks) {
  /* next_tick_to_awake 가 깨워야 할 스레드중 가장 작은 tick을 갖도록 업데이트
   * 한다 */
 ...
 * 					 현재 스레드(우선순위가 높고, lock을 요청한
 * 스레드)의 우선순위 부여
 */
void donate_priority(void) {
  int depth;  // nested의 최대 깊이 지정(MAX_DEPTH = 8)
  struct thread *curr = thread_current();  // lock을 기다리고 있는 현재
                                           // 스레드(우선순위 높은 스레드)
...
/**
 * remove_with_lock - 현재 스레드가 해당 lock을 반납할 때, dontaions에 lock을
 * 요청한 스레드 삭제
 */
void remove_with_lock(struct lock *lock) {
  struct list_elem *e;
...
/**
 * refresh_priority - 현재 스레드의 우선순위를 donations의 가장 큰 값을
 * 우선순위로 update donations가 비어있다면, 원래의 priority로 update
 */
void refresh_priority(void) {
  struct thread *curr = thread_current();
  ...
  if (!list_empty(&curr->donations)) {
    ...
    struct thread *front =
        list_entry(list_front(&curr->donations), struct thread, donation_elem);
