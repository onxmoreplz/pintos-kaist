...
/* timer_sleep - ticks_sleep만큼 해당 스레드를 활성화시키지 않는다. 
 * 				그 사이에 다른 스레드가 CPU를 사용하는 것이 가능해진다. 
 */
void timer_sleep(int64_t ticks_sleep)
{
...
/**
 * timer_interrupt - 매 tick마다 timer 인터럽트 시 호출되는 함수
 * 
 *  sleep_queue에서 깨어날 thread가 있는지 확인
 * 	sleep_queue에서 가장 빨리 깨어날 thread의 tick값 확인
 */
